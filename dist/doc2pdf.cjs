...
var libreofficeConvert = require('libreoffice-convert');
...
async function doc2pdf(args) {
	...
	// Validate inputFile
	if (typeof inputFile !== 'string' && inputFile instanceof node_url.URL === false) {
		throw new TypeError('inputFile must be a string or URL')
	}

	// Read the input DOCX file and create a PizZip instance
	const content = await promises.readFile(inputFile);
	...
	if (typeof outputFile === 'string' || outputFile instanceof node_url.URL) {
		await promises.writeFile(outputFile, pdfBuffer);
		return outputFile
	}
